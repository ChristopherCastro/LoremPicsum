...
$limboColors = $db->Query('SELECT photo_id FROM photo_colors WHERE photo_id NOT IN (SELECT id FROM photos)');
foreach ($limboColors as $color) {
	echo "Color pointing to unexisting photo {$color['photo_id']}\n";
	$errors++;
}
echo !$errors ? "PASS!\n" : "ERROR\n";
