 ...
 *             'amount' => 200,
 *             'percent' => 0.6,
 *             'RGB' => array('red' => 255, 'green' => 0, 'blue' => 170),
 *             'LAB' => array('l' => 56,24, 'a' => 88,11, 'b' => -18,83),
 *             'HSV' => array('hue' => 320.0, 'sat' => 1.0000, 'val' => 1.0000)
 *          ),
 *         'CC0355' => array(
 *         ...
 *     )
 ...
 * - percent: Percent of pixels matching this colors (based on $numColors).
 * - RGB: RGB color space representation (integer).
 * - LAB: CieLAB color space representation (float).
 * - HSV: HSV color space representation (float).
 *
 ...
 */
	public function extract($numColors = 15) {
		...
		$total = array_sum(array_values($_colors));
		
		foreach ($_colors as $hex => $amount) {
			$Color = new Color();
			$Color->fromHex($hex);
			$RGB = $Color->toRgbInt();
			$LAB = $Color->toLabCie();
